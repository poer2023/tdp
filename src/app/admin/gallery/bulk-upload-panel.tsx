...
export function BulkUploadPanel({}: { posts?: PostSummary[] }) {
  ...
  return (
    <section
      ...
      className="rounded-3xl border border-stone-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-stone-800/70 dark:bg-stone-900/70"
    >
      <div className="mb-4 flex items-center justify-between">
        <div className="space-y-1">
          <h2 className="text-lg font-semibold text-stone-900 dark:text-stone-50">批量上传</h2>
      ...
      <form ref={formRef} action={formAction} className="grid gap-6 md:grid-cols-3">
        <div className="space-y-4 md:col-span-2">
          <label
            ...
            className="flex cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-stone-300 p-6 text-center text-sm text-stone-500 transition hover:bg-stone-50 dark:border-stone-700 dark:text-stone-400 dark:hover:bg-stone-900"
          >
            <input
              ref={fileInputRef}
              name="files"
              ...
              onChange={onPick}
            />
            <div className="text-stone-700 dark:text-stone-300">拖拽文件到此处或点击选择</div>
          ...
          {files.length > 0 && (
            <ul className="max-h-60 overflow-auto rounded-lg border border-stone-200 text-sm dark:border-stone-800">
              {files.map((f) => (
                <li
                  key={f.name + f.size}
                  className="flex items-center justify-between border-b border-stone-200 px-3 py-2 last:border-b-0 dark:border-stone-800"
                >
                  <span className="truncate text-stone-700 dark:text-stone-300" title={f.name}>
                    {f.name}
                  </span>
        ...
        <div className="space-y-4">
          ...
          <Field label="关联文章 (可选)">
            <div className="relative">
              ...
              {postOptions.length > 0 && (
                <ul className="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-lg border border-stone-200 bg-white shadow-lg dark:border-stone-700 dark:bg-stone-900">
                  {postOptions.map((o) => (
                    <li key={o.id}>
                      <button
                        type="button"
          ...
          <button
            ...
            className="w-full rounded-lg border border-blue-600 bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-all duration-150 hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-blue-400 dark:bg-blue-400 dark:text-stone-900"
          >
            走队列上传（带进度）
          </button>
        </div>
      ...
      {state.results && state.results.length > 0 && (
        <div className="mt-4 overflow-hidden rounded-xl border border-stone-200 dark:border-stone-800">
          <table className="w-full text-sm">
            ...
            <tbody>
              {state.results.map((r) => (
                <tr key={r.key} className="border-t border-stone-200 dark:border-stone-800">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.map((it, i) => (
                <tr
                  key={it.key}
                  className="border-t border-stone-200 align-middle dark:border-stone-800"
                >
                  <td className="px-3 py-2 font-mono text-xs text-stone-600 dark:text-stone-400">
                    {it.key}
                  </td>
                  <td className="px-3 py-2">
                    <div className="h-2 w-40 overflow-hidden rounded bg-stone-100 dark:bg-stone-800">
                      <div
                  ...
                  <td className="px-3 py-2 text-xs">
                    {it.status === "idle" && <span className="text-stone-500">等待</span>}
                    {it.status === "uploading" && (
                    ...
                    {it.status === "cancelled" && <span className="text-stone-500">已取消</span>}
                  </td>
                  <td className="px-3 py-2">
                    {it.status !== "uploading" && (
                      <button
                        type="button"
                        onClick={() => uploadOne(i)}
                        className="mr-2 rounded border border-stone-300 px-2 py-1 text-xs text-stone-700 hover:bg-stone-50 dark:border-stone-700 dark:text-stone-300 dark:hover:bg-stone-900"
                      >
                    ...
                    {it.status === "uploading" && (
                      <button
                        type="button"
                        onClick={() => cancelOne(i)}
                        className="rounded border border-stone-300 px-2 py-1 text-xs text-stone-700 hover:bg-stone-50 dark:border-stone-700 dark:text-stone-300 dark:hover:bg-stone-900"
                      >
                        取消
                      </button>
                    )}
