...
export async function getFootprint(id: string): Promise<Footprint | null> {
    return withDbFallback(
        async () => prisma.footprint.findUnique({ where: { id } }),
        async () => null
    );
}
...
export async function getFootprintStats(): Promise<FootprintStats> {
    const emptyStats: FootprintStats = {
        ...
        totalDuration: 0,
        byType: {
            WALK: { count: 0, distance: 0 },
            RUN: { count: 0, distance: 0 },
            BIKE: { count: 0, distance: 0 },
            DRIVE: { count: 0, distance: 0 },
            TRANSIT: { count: 0, distance: 0 },
            FLIGHT: { count: 0, distance: 0 },
    ...
    return withDbFallback(
        async () => {
            ...
            const byType: FootprintStats["byType"] = { ...emptyStats.byType };
