...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const refreshGallery = async () => {
        ...
        try {
            ...
            if (res.ok && Array.isArray(data.images)) {
                ...
                if (typeof data.total === 'number') setGalleryCount(data.total);
                else setGalleryCount(data.images.length);
            }
        } catch (error) {
    ...
    // Analytics removed - using Cloudflare Web Analytics
    const refreshAnalytics = async (_period: '7d' | '30d' | '90d' | 'all' = '30d') => {
        // No-op: Analytics moved to Cloudflare Dashboard
    };
    ...
    const addGalleryItem = async (item: GalleryUploadInput) => {
        try {
            ...
            const mapped = data.images ? mapApiGalleryItem(data.images[0]) : mapApiGalleryItem(data);
